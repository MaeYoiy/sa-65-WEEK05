...
func CreateResearchRoomReservationRecord(c *gin.Context) {
	...
	if tx := entity.DB().Where("id = ?", researchroomreservationrecord.TimeID).First(&time); tx.RowsAffected == 0 {
	...
	RRRR := entity.ResearchRoomReservationRecord{
		...
		BookDate:     researchroomreservationrecord.BookDate, // ตั้งค่าฟิลด์ watchedTime
...
func GetResearchRoomReservationRecord(c *gin.Context) {
	var researchroomreservationrecord entity.ResearchRoomReservationRecord
	id := c.Param("id")
	if err := entity.DB().Preload("ResearchRoom").Preload("User").Preload("AddOn").Preload("Time").Raw("SELECT * FROM research_room_reservation_records WHERE id = ?", id).Find(&researchroomreservationrecord).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListResearchRoomReservationRecords(c *gin.Context) {
	...
	if err := entity.DB().Preload("ResearchRoom").Preload("User").Preload("AddOn").Preload("Time").Raw("SELECT * FROM research_room_reservation_records").Find(&researchroomreservationrecords).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
